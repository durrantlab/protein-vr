define(["require", "exports", "../Objects/Ground", "../Objects/Skybox", "../CameraChar", "../Environment", "./RenderLoop", "./MouseState", "./Sound", "../Settings/UserVars", "../Events/Actions/MoveCamera", "../Events/Event", "../Events/TriggerConditionals/ClickedObject"], function (require, exports, Ground_1, Skybox_1, CameraChar, Environment, RenderLoop, MouseState, Sound, UserVars, MoveCamera_1, Event_1, ClickedObject_1) {
    ...
    */
                        ...
                        newScene.executeWhenReady(function () {
                            ...
                            for (var meshIdx = 0; meshIdx < PVRGlobals.scene.meshes.length; meshIdx++) {
                                ...
                                if (this.proteinVRInfo["materials"][m.name] !== undefined) {
                                    var mat_inf = this.proteinVRInfo["materials"][m.name];
                            ...
                            PVRGlobals.scene.meshes.forEach(function (m) {
                                ...
                                if (m.name.indexOf("Decimated") !== -1) {
                                    var nameToUse = m.name.replace(/Decimated/g, "");
                                    var parentMesh = PVRGlobals.meshesByName[nameToUse];
    ...
    exports.setup = setup;
    // }
    function continueSetupAfterSettingsPanelClosed() {
        if (sceneLoadedAndReady === false) {
            ...
            // Set up sounds
            for (var soundIdx = 0; soundIdx < proteinVRInfo["sounds"].length; soundIdx++) {
                var sound = proteinVRInfo["sounds"][soundIdx];
                var filename = sound[0];
