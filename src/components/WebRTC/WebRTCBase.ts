 ...
 * @param  {*}      webRTCBase  The webRTCBase class. I'm not making this
 *                              function a member of that class because when I
 *                              do, closure compiler removes it. Hours of
 *                              effort went into making this very annoying
 *                              discovery.
 * @param  {string} assignedId  Use this id if specified. Otherwise, generate
 *                              one.
 * @returns Promise<any>        A promise that is resolved when the peer is
 *                              open and has returned an id.
 */
function createPeerObj(webRTCBase: WebRTCBase, assignedId?: string): Promise<any> {
    ...
    if (assignedId === undefined) {
        const wrds = ["act", "add", "age", "ago", "aid", "aim", "air", "all",
                    ...
                    "win", "yes", "yet", "you"];
        assignedId = "pvr" + webRTCBase.randomNumStr();
        assignedId += wrds[Math.floor(Math.random() * wrds.length)] + webRTCBase.randomNumStr();
        // assignedId += wrds[Math.floor(Math.random() * wrds.length)] + webRTCBase.randomNumStr();
    ...
    webRTCBase.peer = new Peer(assignedId, {
        "debug": 2,
        "config": {
            'iceServers': [
    ...
    return new Promise((resolve, reject) => {
        ...
        webRTCBase.peer["on"]("open", (id: string) => {
            // Workaround for peer.reconnect deleting previous id
            if (webRTCBase.peer["id"] === null) {
                webRTCErrorMsg("Received null id from peer open.");
                webRTCBase.peer["id"] = webRTCBase.peerId;
            } else {
                webRTCBase.peerId = webRTCBase.peer["id"];
            }

            resolve(webRTCBase.peerId);

            if (DEBUG === true) {
                console.log("WEBRTC: ", webRTCBase.peerId);
            }
        });
    });
}
...
export class WebRTCBase {
    ...
    constructor(assignedId?: string) {
        this.peerOpenPromise = createPeerObj(this, assignedId);
