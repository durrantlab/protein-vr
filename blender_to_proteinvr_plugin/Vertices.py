...
def simplify_big_meshes(params):
    ...
    for obj in bpy.data.objects:
        ...
        if num_verts_in_obj > params["max_obj_verts_allowed"]:
            print("\t" + obj.name + " has " + str(num_verts_in_obj) + " verticies! The maximum allowed is " + str(params["max_obj_verts_allowed"]) + ". Decimating...")
            ratio = params["max_obj_verts_allowed"]/num_verts_in_obj
            Utils.decimate_obj(obj, ratio)

def simplify_meshes_until_scene_vert_count_ok(params):
    ...
    while tot_num_verts > params["max_scene_verts_allowed"]:
        for obj in Utils.decimatable_objs(params):
