...
class ProteinVR(PanelParentClass):
    ...
    def draw(self, context):
        ...
        obj_to_use_name = (
            obj_to_use.name   
            if not obj_to_use.name.startswith("ProteinVR_highres_sphere__")
            else obj_to_use.name.split("__")[1]
        )

        if obj_to_use.name.startswith("ProteinVR_highres_sphere__"):
    ...
    def draw_no_object_selected_start_over(self):
        ...
        for obj in bpy.data.objects:
            if obj.name.startswith("ProteinVR_"):
    ...
    def draw_citation(self):
        ...
        self.ui.label("If you use ProteinVR, please cite:")
    ...
    def draw_main_protein_mesh_panel(self):
        ...
        spheres = [obj for obj in bpy.data.objects if obj.name.startswith("ProteinVR_highres_sphere__")]
...
class OBJECT_OT_AddSphereButton(ButtonParentClass):
    ...
    def execute(self, context):
        ...
        if (cursor_loc.x > a_min.x - margin and 
            ...
            sphere_name = "ProteinVR_highres_sphere__" + obj.name + "__" + str(0)
            i = 0
            while sphere_name in bpy.data.objects.keys():
                i = i + 1
                sphere_name = "ProteinVR_highres_sphere__" + obj.name + "__" + str(i)
...
class OBJECT_OT_RemoveAnimations(ButtonParentClass):
    ...
    def execute(self, context):
        ...
        for obj in bpy.data.objects:
            if (
                obj.name.startswith("ProteinVR_") and 
                not obj.name.startswith("ProteinVR_highres_sphere__")
            ):
                obj.select = True
                bpy.ops.object.delete()
