...
class ProcessTrajectory(BackgroundJobParentClass):
    ...
    def add_empties_at_atom_points(self):
        ...
        guide_empties.name = "ProteinVR_GuideEmpties"
        ...
        for index in self.selection_atoms_to_keep:
            ...
            empty.name = "ProteinVR_empty" + str(index)
    ...
    def get_pruned_indecies(self):
        ...
        for sphere in [obj for obj in bpy.data.objects if obj.name.startswith("ProteinVR_highres_sphere__")]:
    ...
    def position_empties_at_atom_locs(self, position_all=False):
        ...
        for coor_index, coor in enumerate(coors):
            si = sel[coor_index]
            empty = bpy.data.objects["ProteinVR_empty" + str(si)]
    ...
    def armature_and_bones(self):
        ...
        object.name = 'ProteinVR_Armature'
        armature = object.data
        armature.name = 'Frame'

        # Add bones
        for index in self.selection_atoms_to_keep:
            bone_name = 'ProteinVR_bone' + str(index)
        ...
        armature = bpy.data.objects["ProteinVR_Armature"]

        for index in self.selection_atoms_to_keep:
            bone = armature.pose.bones['ProteinVR_bone' + str(index)]
            constraint = bone.constraints.new(type="COPY_LOCATION")
            constraint.target = bpy.data.objects["ProteinVR_empty" + str(index)]
        ...
        bpy.context.scene.objects.active = bpy.data.objects['ProteinVR_Armature']
